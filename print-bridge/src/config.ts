...
const defaultConfig: BridgeConfig = {
  ...
  backends: [
    {
      name: 'RN POS',
      url: 'https://conestonejuly-2026-production.up.railway.app',
      enabled: true
    },
    {
      name: 'QR POS',
      url: 'https://qrcode-conestone-production.up.railway.app',
      enabled: true
    }
  ]
};
...
function loadConfig(): BridgeConfig {
  ...
  try {
    ...
    if (!parsed.backends || !Array.isArray(parsed.backends)) {
      const url = parsed.apiUrl || parsed.backendUrl || 'https://conestonejuly-2026-production.up.railway.app';
      parsed.backends = [
        {
          name: 'Default',
          url: url,
